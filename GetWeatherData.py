...
def GetWeatherData(city, num_days):
    """
    获取指定城市最近num_days天的历史天气数据并保存为Excel文件。

    :param city: 城市名称，例如 'shenzhen'
    :param num_days: 需要获取的天数
    """
    # 计算近num_days天的起止日期
    today = datetime.now()
    ...
    # 写入Excel，并传递city和num_days参数
    _write_to_excel(filtered_data, city, num_days)
...
def _write_to_excel(data, city, num_days):
    """
    将天气数据写入Excel文件。
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = f"Last{num_days}DaysWeather"
    # 写入表头
    ...
    # 保存文件，命名为 {city}Last{num_days}DaysWeather.xlsx
    if not os.path.exists("output"):
        os.mkdir("output")
    excel_filename = f"{city}Last{num_days}DaysWeather.xlsx"
    wb.save(f'output/{excel_filename}')
    logging.info(f"数据已成功写入 output/{excel_filename}")


if __name__ == '__main__':
    GetWeatherData('guangzhou', 1000)
